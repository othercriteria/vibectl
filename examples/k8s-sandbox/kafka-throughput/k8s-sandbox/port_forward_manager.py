...
REMOTE_PORT = os.environ.get("REMOTE_PORT", "9092") # Usually same as local for this use case
...
PKILL_PATTERN = (
    f"kubectl port-forward.*-n {TARGET_NAMESPACE}.*svc/{TARGET_SERVICE}.*{LOCAL_PORT}:{REMOTE_PORT}"
)
...
kubectl_process: Optional[subprocess.Popen[Any]] = None
...
def signal_handler(signum: int, frame: Any) -> None:
    ...
    if kubectl_process and kubectl_process.poll() is None: # Check if process exists and is running
        logger.info(f"Sending SIGTERM to kubectl process (PID: {kubectl_process.pid})...")
        ...
        try:
            ...
            logger.warning("kubectl process did not terminate in time. Sending SIGKILL...")
...
def kill_existing_port_forwards() -> None:
    ...
    try:
        ...
        # We need to be careful not to kill ourself if this script is run with similar args
        ...
        if result.returncode == 0:
            ...
            logger.info("No existing port-forward processes found matching the pattern.")
        else: # Other error
            logger.warning(
    ...
    try:
        with open(KUBECTL_LOG_FILE, "ab") as klogf: # Append binary mode
            ...
            kubectl_process = subprocess.Popen(
                cmd, stdout=klogf, stderr=klogf
            )
        logger.info(f"kubectl port-forward process started with PID: {kubectl_process.pid}")
        return True
    except FileNotFoundError:
        logger.error(f"'{KUBECTL_CMD}' command not found. Please ensure it is installed and in PATH.")
        return False
    except Exception as e:
...
def main_loop() -> None:
    ...
    logger.info(f"  Target: {TARGET_NAMESPACE}/svc/{TARGET_SERVICE}:{REMOTE_PORT} -> localhost:{LOCAL_PORT}")
    ...
    while True:
        ...
        if not start_port_forward():
            logger.error("Failed to start port-forward. Retrying in {RETRY_DELAY_SECONDS}s...")
        ...
        if kubectl_process:
            ...
            # If we reach here, kubectl_process.poll() is not None, meaning it terminated
            exit_code = kubectl_process.returncode
            logger.warning(
                f"kubectl port-forward process (PID: {kubectl_process.pid}) terminated with exit code: {exit_code}."
            )
        else: # Should not happen if start_port_forward returned True
            logger.error("kubectl_process is None after a successful start attempt. This is unexpected.")
...
if __name__ == "__main__":
    ...
    if not all([TARGET_NAMESPACE, TARGET_SERVICE, LOCAL_PORT, REMOTE_PORT]):
        logger.error(
            "One or more required environment variables (TARGET_NAMESPACE, TARGET_SERVICE, LOCAL_PORT, REMOTE_PORT) are not set."
        )
        sys.exit(1)
