...
echo "🧹 Cleaning up stale files in ${STATUS_DIR} (preserving ${CLUSTER_ID_FILE})..."
# Loop through files in STATUS_DIR and delete them if they are not the CLUSTER_ID_FILE
for f in "${STATUS_DIR}"/*; do
    # Check if it is a file and not the one we want to preserve
    if [ -f "$f" ] && [ "$f" != "${CLUSTER_ID_FILE}" ]; then
...
# Ensure TARGET_LATENCY_MS from the calling environment is passed, with a default
TARGET_LATENCY_MS_VALUE=${TARGET_LATENCY_MS:-10.0}

cat > "${ENV_FILE}" << EOF
# Generated by run.sh - DO NOT EDIT
DOCKER_GID=${DOCKER_GID}
GENERATED_KAFKA_CLUSTER_ID=${GENERATED_KAFKA_CLUSTER_ID}
TARGET_LATENCY_MS=${TARGET_LATENCY_MS_VALUE}
EOF
