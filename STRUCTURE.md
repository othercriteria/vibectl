...
- `uv.lock` - Canonical locked dependency versions generated by uv (committed to version control)
- `pylock.toml` - Derived TOML v1 lockfile generated automatically for tool compatibility
...
- **Update lock file**: Run `make lock` to regenerate `uv.lock` (and derived `pylock.toml`) with resolved versions
- **Install dependencies**: The Nix dev shell automatically installs from `uv.lock`

#### Key Commands

- `make lock` - Regenerate uv.lock and pylock.toml files from pyproject.toml
- `uv lock --upgrade` - Direct uv command to refresh uv.lock
