...
pytestmark = pytest.mark.filterwarnings(
    'ignore:install \\"ipywidgets\\" for Jupyter support:UserWarning:rich.live'
)
...
def test_handle_vibe_request_port_forward_clean(
    ...
    with patch("vibectl.command_handler._execute_command") as mock_execute_command:
        ...
        ) as mock_live_handler:
            mock_live_handler.side_effect = AssertionError(
                "Live handler called unexpectedly!"
            )
...
def test_handle_vibe_request_port_forward_with_flags(
    ...
    with patch("vibectl.command_handler._execute_command") as mock_execute_command:
        mock_execute_command.return_value = Success(
            data="Forwarding...Output with Flags"
        )
...
def test_execute_port_forward_success(
    ...
    mock_process.stdout.readline = AsyncMock(
        side_effect=[b"Forwarding from 127.0.0.1:8080 -> 80\\n", b""]
    )
    ...
    with (
        ...
        # Restore necessary mocks for internal calls, remove unused 'as' assignments
        patch("vibectl.live_display.update_memory"),
        patch("vibectl.live_display.get_model_adapter") as mock_get_adapter_live,
    ):  # Mock adapter in live_display
        # Configure the nested mock adapter
        mock_adapter_live = MagicMock()
        mock_model_live = Mock()
        mock_adapter_live.get_model.return_value = mock_model_live
        mock_get_adapter_live.return_value = mock_adapter_live
        ...
        result = _execute_port_forward_with_live_display(
            ...
            summary_prompt_func=summary_func,
        )
        ...
        expected_cmd = [
            ...
            "port-forward",
            resource,
            *args,  # Add other expected args like --address if needed
        ]
...
def test_execute_port_forward_kubectl_error(
    ...
    mock_process.stdout.readline = AsyncMock(
        return_value=b""
    )  # No stdout expected on error start
    mock_process.stderr = AsyncMock()
    mock_process.stderr.read = AsyncMock(
        return_value=kubectl_error_message
    ...
    with (
        ...
        result = _execute_port_forward_with_live_display(
            ...
            remote_port=remote_port,
            display_text=display_text,
            summary_prompt_func=summary_func_err,
        )
        ...
        expected_cmd = [
            ...
            "port-forward",
            resource,
            *args,  # Add other expected args if needed
        ]
        call_args = mock_create_subprocess.call_args.args
        assert call_args[: len(expected_cmd)] == tuple(expected_cmd)
